...
void MoveCamera(GAME_VECTOR *ideal, int32_t speed)
{
	...
	FLOOR_INFO *floor = GetFloor(g_Camera.pos.x, g_Camera.pos.y, g_Camera.pos.z,
	...
	if (g_Camera.pos.y >= height && ideal->y >= height)
	{
		...
		floor = GetFloor(g_Camera.pos.x, g_Camera.pos.y, g_Camera.pos.z,
						 &g_Camera.pos.room_number);
		height =
			GetHeight(floor, g_Camera.pos.x, g_Camera.pos.y, g_Camera.pos.z) -
			GROUND_SHIFT;
	...
	GetFloor(g_Camera.pos.x, g_Camera.pos.y + g_Camera.shift, g_Camera.pos.z,
			 &g_Camera.pos.room_number);

	phd_LookAt(g_Camera.pos.x, g_Camera.pos.y + g_Camera.shift, g_Camera.pos.z,
			   g_Camera.target.x, g_Camera.target.y, g_Camera.target.z, 0);

	g_Camera.actual_angle = phd_atan(g_Camera.target.z - g_Camera.pos.z,
...
void ShiftCamera(int32_t *x, int32_t *y, int32_t target_x, int32_t target_y,
				 int32_t left, int32_t top, int32_t right, int32_t bottom)
{
	int32_t shift;
...
void SmartShift(GAME_VECTOR *ideal,
				void (*shift)(int32_t *x, int32_t *y, int32_t target_x,
							  int32_t target_y, int32_t left, int32_t top,
							  int32_t right, int32_t bottom))
{
	...
	if (ideal->z < left && bad_left)
	{
		...
		{
			shift(&ideal->z, &ideal->x, g_Camera.target.z, g_Camera.target.x,
	...
	if (noclip)
	{
		if (ideal->x < top && bad_top)
		{
			...
			{
				shift(&ideal->x, &ideal->z, g_Camera.target.x,
					  g_Camera.target.z, top, left, bottom, right);
			}
			else
			{
				shift(&ideal->x, &ideal->z, g_Camera.target.x,
					  g_Camera.target.z, top, right, bottom, left);
			}
		}
		else if (ideal->x > bottom && bad_bottom)
		{
			...
			{
				shift(&ideal->x, &ideal->z, g_Camera.target.x,
					  g_Camera.target.z, bottom, left, top, right);
			}
			else
			{
				shift(&ideal->x, &ideal->z, g_Camera.target.x,
					  g_Camera.target.z, bottom, right, top, left);
			}
		}
	}
...
int32_t ShiftClamp(GAME_VECTOR *pos, int32_t clamp)
{
	...
	if (z < box->left + clamp && BadPosition(x, y, z - clamp, pos->room_number))
	{
		pos->z = box->left + clamp;
	}
	else if (z > box->right - clamp &&
			 BadPosition(x, y, z + clamp, pos->room_number))
	{
		pos->z = box->right - clamp;
	}

	if (x < box->top + clamp && BadPosition(x - clamp, y, z, pos->room_number))
	{
		pos->x = box->top + clamp;
	}
	else if (x > box->bottom - clamp &&
...
void LookCamera(ITEM_INFO *item)
{
	...
	if (BadPosition(g_Camera.target.x, g_Camera.target.y, g_Camera.target.z,
...
void CalculateCamera()
{
	...
	int32_t fixed_camera = g_Camera.item && (g_Camera.type == CAM_FIXED ||
	...
	if (!fixed_camera)
	{
		y += bounds[FRAME_BOUND_MAX_Y] +
			 ((bounds[FRAME_BOUND_MIN_Y] - bounds[FRAME_BOUND_MAX_Y]) * 3 >> 2);
	}
	else
	{
	...
	if (g_Camera.item && !fixed_camera)
	{
		...
		int16_t shift = phd_sqrt(SQUARE(g_Camera.item->pos.z - item->pos.z) +
								 SQUARE(g_Camera.item->pos.x - item->pos.x));

		int16_t angle = phd_atan(g_Camera.item->pos.z - item->pos.z,
								 g_Camera.item->pos.x - item->pos.x) -
						item->pos.y_rot;

		int16_t tilt = phd_atan(
			shift,
			y - (g_Camera.item->pos.y +
				 (bounds[FRAME_BOUND_MIN_Y] + bounds[FRAME_BOUND_MAX_Y]) / 2));
	...
	else
	{
		...
		FLOOR_INFO *floor =
			GetFloor(g_Camera.target.x, g_Camera.target.y, g_Camera.target.z,
					 &g_Camera.target.room_number);

		if (g_Camera.target.y > GetHeight(floor, g_Camera.target.x,
										  g_Camera.target.y, g_Camera.target.z))
		{
			g_ChunkyFlag = false;
		}
...
void CameraOffsetAdditionalElevation(int16_t delta)
{
	...
	if (delta > 0)
	{
		if (g_Camera.additional_elevation > INT16_MAX_TR - delta)
		{
			g_Camera.additional_elevation = INT16_MAX_TR;
		}
		else
		{
	...
	{
		if (g_Camera.additional_elevation < INT16_MIN_TR - delta)
		{
			g_Camera.additional_elevation = INT16_MIN_TR;
		}
		else
		{
			g_Camera.additional_elevation += delta;
