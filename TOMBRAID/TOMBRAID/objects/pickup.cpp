...
void PickUpCollision(int16_t item_num, ITEM_INFO *lara_item, COLL_INFO *coll)
{
    ...
    if (g_Lara.water_status == LWS_ABOVEWATER)
	{
        ...
        if (lara_item->current_anim_state == AS_PICKUP)
		{
			...
			{
                g_Lara.mesh_ptrs[LM_TORSO] = g_Meshes[g_Objects[O_SHOTGUN].mesh_index + LM_TORSO];
            }
        ...
        if (g_Input.action && g_Lara.gun_status == LGS_ARMLESS && !lara_item->gravity_status && lara_item->current_anim_state == AS_STOP)
		{
