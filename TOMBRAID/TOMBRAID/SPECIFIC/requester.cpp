...
int32_t DisplayRequester(REQUEST_INFO *req)
{
    ...
    int32_t box_height = req->line_height + lines_height + BOX_PADDING * 2 + BOX_BORDER * 2;
    ...
    if (!req->heading)
	{
        req->heading = Text_Create(req->x, line_one_off - req->line_height - BOX_PADDING, req->heading_text);
		Text_CentreH(req->heading, 1);
        Text_AlignBottom(req->heading, 1);
        Text_AddBackground(req->heading, req->pix_width - 2 * BOX_BORDER, 0, 0, 0);
    ...
    for (int i = 0; i < line_qty; i++)
	{
		...
		{
            Text_AddBackground(req->texts[i], req->pix_width - BOX_PADDING - 1 * BOX_BORDER, 0, 0, 0);
    ...
    if (req->line_offset != req->line_old_offset)
	{
        for (int i = 0; i < line_qty; i++)
		{
            if (req->texts[i])
			{
                Text_ChangeText(req->texts[i], &req->item_texts[req->item_text_len * (req->line_offset + i)]);
            }
        }
    }
    ...
    if (g_InputDB.select)
	{
        if ((req->item_flags[req->requested] & RIF_BLOCKED) && (req->flags & RIF_BLOCKABLE))
		{
			g_Input.any = 0;
            return 0;
        }
