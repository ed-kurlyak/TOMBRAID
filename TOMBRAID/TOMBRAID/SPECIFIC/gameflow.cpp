...
void Init_GameFlow()
{
	...
	if (select_game == VER_TR1)
	{
		...
		// ¬ оригинальной игре (OG) есть специальный уровень под названием LV_CURRENT дл€ обработки логики сохранени€/загрузки.
		// ¬ T1M этот трюк больше не используетс€. ќднако существующие сохранЄнные игры
		// ожидают, что количество уровней будет совпадать, иначе игра вылетит.
		// ѕоэтому добавлен этот фиктивный уровень.
...
int Start_New_Game(int LevelNum)
{
	...
	//new for tr1
	//создаем палитру после загрузки уровн€
	//т.е. данных о палитре из файла уровн€
	//Create_BackBuffer();
