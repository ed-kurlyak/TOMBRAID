    ...
    if (!m_TextB[0]) {
        ...
        for (const TEXT_COLUMN_PLACEMENT *col = cols;
            ...
            const char *scancode_name = S_Input_GetKeyCodeName(
                S_Input_GetAssignedKeyCode(g_Config.input.layout, (INPUT_KEY) col->option));
	...
	*/
    ...
    for (const TEXT_COLUMN_PLACEMENT *item1 = cols; item1->col_num != -1;
        ...
        S_INPUT_KEYCODE key_code1 =
            S_Input_GetAssignedKeyCode(g_Config.input.layout, (INPUT_KEY)item1->option);
...
void Option_DefaultConflict()
{
    for (int i = 0; i < INPUT_KEY_NUMBER_OF; i++) {
        ...
        for (int j = 0; j < INPUT_KEY_NUMBER_OF; j++) {
            if (key_code == S_Input_GetAssignedKeyCode(INPUT_LAYOUT_USER, (INPUT_KEY)j)) {
	...
	*/
    ...
    case 2: {
        ...
        if (key_code >= 0 && scancode_name) {
            S_Input_AssignKeyCode(g_Config.input.layout, (INPUT_KEY)m_KeyChange, key_code);
    ...
    case 3: {
        S_INPUT_KEYCODE key_code =
            S_Input_GetAssignedKeyCode(g_Config.input.layout, (INPUT_KEY)m_KeyChange);
