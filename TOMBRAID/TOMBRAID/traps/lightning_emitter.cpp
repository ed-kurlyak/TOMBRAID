...
void DrawLightning(ITEM_INFO *item)
{
    ...
    phd_TranslateRel(frmptr[0][FRAME_POS_X], frmptr[0][FRAME_POS_Y], frmptr[0][FRAME_POS_Z]);

    int32_t x1 = g_PhdMatrixPtr->_03;
    int32_t y1 = g_PhdMatrixPtr->_13;
    int32_t z1 = g_PhdMatrixPtr->_23;

    Output_DrawPolygons(g_Meshes[g_Objects[O_LIGHTNING_EMITTER].mesh_index], clip);
