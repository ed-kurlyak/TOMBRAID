...
void FlyerControl(int16_t item_num)
{
	...
	CREATURE_INFO *flyer = (CREATURE_INFO *)item->data;
	int16_t head = 0;
	int16_t angle = 0;
	...
	else
	{
		...
		if (item->object_number == O_WARRIOR1)
		{
			...
			else if ((info.zone_number != info.enemy_zone && !shoot1 &&
					  !shoot2 && (!info.ahead || flyer->mood == MOOD_BORED)) ||
					 flyer->mood == MOOD_ESCAPE)
			{
				flyer->flags |= FLYER_FLYMODE;
			}
		...
		case FLYER_POSE:
			...
			else if (flyer->mood == MOOD_ATTACK || flyer->mood == MOOD_ESCAPE)
			{
				item->goal_anim_state = FLYER_STOP;
			}
			break;
