...
void LionControl(int16_t item_num)
{
	...
	CREATURE_INFO *lion = (CREATURE_INFO *)item->data;
	...
	if (item->hit_points <= 0)
	{
		if (item->current_anim_state != LION_DEATH)
		{
			...
			{
				item->anim_number = g_Objects[O_PUMA].anim_index +
									PUMA_DIE_ANIM +
									(int16_t)(Random_GetControl() / 0x4000);
			}
			else if (item->object_number == O_LION)
			{
				item->anim_number = g_Objects[O_LION].anim_index +
									LION_DIE_ANIM +
									(int16_t)(Random_GetControl() / 0x4000);
			}
			else
	...
	else
	{
		...
		if (info.ahead)
		{
			//info.angle угол между направлением врага и Ларой
			head = info.angle;
		}
