...
int32_t Display_Inventory(int inv_mode)
{
        ...
        do
        {
                ...
                else
                {
                        ...
                        {
                                g_OldInputDB = input; // в этом случае точно ноль
                        }
                }

				g_InputDB = input;

                if (g_InvMode == INV_TITLE_MODE && g_Input == 0) // INV_TITLE_MODE
                {
                        g_NoInputCount += 1;
				...
				//no demo- has widescreen bug
				g_NoInputCount = 0;
				g_ResetFlag = 0;

                for (i = 0; i < m_InvNFrames; i++)
                {
                        if (g_IDelay)
                        {
                                if (g_IDCount)
                ...
                PHD_ANGLE angle = 0;
                for (i = 0; i < ring.number_of_objects; i++)
                {
                        ...
                        if (i == ring.current_object)
                        {
                                                         ...
                                                         imo.status == RNG_CLOSING_ITEM)
                                        {
                                                ...
                                                if (inv_item->y_rot != inv_item->y_rot_sel)
                                                {
                                                        ...
                                                        inv_item->y_rot &= 0xFC00u;
                                                         ...
                                                         (!(g_Input & IN_LEFT) && !(g_Input & IN_RIGHT)) || !(g_Input & IN_LEFT))
                                        {
                                                g_LsAdder = HIGH_LIGHT;
                                                inv_item->y_rot += 256;
                                        }
                                ...
                                if ((imo.status == RNG_OPEN || imo.status == RNG_SELECTING ||
                                         ...
                                         imo.status == RNG_DESELECT ||
                                         imo.status == RNG_CLOSING_ITEM) &&
                                        !ring.rotating && !(g_Input & IN_LEFT) && !(g_Input & IN_RIGHT))
                                {
                ...
                case RNG_OPEN:
                        ...
                        if (g_Input & IN_RIGHT && ring.number_of_objects > 1)
                        {
                        ...
                        if (g_Input & IN_LEFT && ring.number_of_objects > 1)
                        {
                        ...
                        if ((g_ResetFlag || g_InputDB & IN_OPTION) &&
                        ...
                        if (g_InputDB & IN_FORWARD && g_InvMode != INV_TITLE_MODE &&
                        ...
                        else if (g_InputDB & IN_BACK && g_InvMode != INV_TITLE_MODE &&
                                         g_InvMode != INV_KEYS_MODE)
                        {
...
void Construct_Inventory()
{
        ...
        */
        ...
        for (i = 0; i < g_InvOptionObjects; i++)
        {
