...
int16_t GetCeiling(FLOOR_INFO *floor, int32_t x, int32_t y, int32_t z)
{
        ...
        if (f->index)
        {
                ...
                if (type == FT_ROOF)
                {
                        ...
                        if (!g_ChunkyFlag || (xoff >= -2 && xoff <= 2 && yoff >= -2 && yoff <= 2))
                        {
                                if (xoff < 0)
                                {
                                        height += (int16_t)((xoff * (z & (WALL_L - 1))) >> 2);
...
int32_t GetSecretCount()
{
        ...
        for (int32_t i = 0; i < g_RoomCount; i++)
        {
                ...
                for (int32_t j = 0; j < r->y_size * r->x_size; j++, floor++)
                {
                        int32_t k = floor->index;
