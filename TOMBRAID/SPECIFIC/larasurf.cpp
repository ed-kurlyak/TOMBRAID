...
void LaraAsSurfSwim(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	// if (!g_Config.enable_tr3_sidesteps || !g_Input & IN_SLOW)
	{
		if (g_Input & IN_LEFT)
		{
...
void LaraAsSurfLeft(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	/*
if (g_Config.enable_tr3_sidesteps && g_Input & IN_SLOW && g_Input.left)
	{
	item->fall_speed += 8;
	if (item->fall_speed > SURF_MAXSPEED)
			{
...
void LaraAsSurfRight(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	/*
if (g_Config.enable_tr3_sidesteps && g_Input & IN_SLOW && g_Input & IN_RIGHT)
	{
	item->fall_speed += 8;
	if (item->fall_speed > SURF_MAXSPEED)
			{
...
void LaraAsSurfTread(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	if (g_Input & IN_LOOK)
	{
		...
		if (g_Input & IN_LEFT && g_Lara.head_y_rot > -MAX_HEAD_ROTATION_SURF)
		{
			g_Lara.head_y_rot -= HEAD_TURN_SURF;
		}
		else if (g_Input & IN_RIGHT && g_Lara.head_y_rot < MAX_HEAD_ROTATION_SURF)
		{
			g_Lara.head_y_rot += HEAD_TURN_SURF;
		}
		g_Lara.torso_y_rot = g_Lara.head_y_rot / 2;

		if (g_Input & IN_FORWARD && g_Lara.head_x_rot > MIN_HEAD_TILT_SURF)
		{
			g_Lara.head_x_rot -= HEAD_TURN_SURF;
		}
		else if (g_Input & IN_BACK && g_Lara.head_x_rot < MAX_HEAD_TILT_SURF)
		{
			g_Lara.head_x_rot += HEAD_TURN_SURF;
		}
		g_Lara.torso_x_rot = 0;
	...
	if (g_Input & IN_STEPL
		/*|| (g_Config.enable_tr3_sidesteps && g_Input & IN_SLOW && g_Input.left)*/)
	{
		item->goal_anim_state = AS_SURFLEFT;
	}
	else if (
		g_Input & IN_STEPR
		/*|| (g_Config.enable_tr3_sidesteps && g_Input & IN_SLOW && g_Input & IN_RIGHT)*/)
	{
	...
	{
		g_Lara.dive_count++;
		if (g_Lara.dive_count == DIVE_COUNT)
		{
...
void LaraSurfaceCollision(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	GetCollisionInfo(coll, item->pos.x, item->pos.y + SURF_HITE, item->pos.z,
					 item->room_number, SURF_HITE);

	ShiftItem(item, coll);

	if ((coll->coll_type & (COLL_FRONT | COLL_LEFT | COLL_RIGHT | COLL_TOP |
							COLL_TOPFRONT | COLL_CLAMP)) ||
		coll->mid_floor < 0)
	{
		item->fall_speed = 0;
