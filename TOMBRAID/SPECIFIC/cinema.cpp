...
{
	StartCinematic(level_num);
	CinematicLoop();
	return StopCinematic(level_num);
}

int32_t StartCinematic(int32_t level_num)
{
	Initialise_Level(level_num);
...
void InitCinematicRooms()
{
    ...
    for (room_num = 0; room_num < g_RoomCount; room_num++) {
...
void CalculateCinematicCamera()
{
    ...
    phd_AlterFOV(fov);
    phd_LookAt(campos.x, campos.y, campos.z, camtar.x, camtar.y, camtar.z, roll);
}
...
void InGameCinematicCamera()
{
    ...
    phd_LookAt(g_Camera.pos.x, g_Camera.pos.y, g_Camera.pos.z, g_Camera.target.x,
        g_Camera.target.y, g_Camera.target.z, roll);
    GetFloor(g_Camera.pos.x, g_Camera.pos.y, g_Camera.pos.z,
