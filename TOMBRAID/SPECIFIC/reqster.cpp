...
int32_t DisplayRequester(REQUEST_INFO *req)
{
	...
	if (!req->heading)
	{
		...
		Text_AddBackground(req->heading, req->pix_width - 2 * BOX_BORDER, 0, 0,
						   0);
	...
	for (int i = 0; i < line_qty; i++)
	{
		...
		{
			Text_AddBackground(req->texts[i],
							   req->pix_width - BOX_PADDING - 1 * BOX_BORDER, 0,
							   0, 0);
	...
	if (g_InputDB & IN_SELECT)
	{
		...
		if ((req->item_flags[req->requested] & RIF_BLOCKED) &&
			...
			*/
		if (!strncmp(req->texts[req->requested - req->line_offset]->string, g_GameFlow.strings[GS_MISC_EMPTY_SLOT_FMT], 12) &&
			!strcmp(g_PassportText->string, g_GameFlow.strings[GS_PASSPORT_LOAD_GAME]))
		{
			g_Input = 0;
			return 0;
		}
