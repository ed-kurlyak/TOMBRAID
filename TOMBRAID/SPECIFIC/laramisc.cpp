...
void LaraControl(int16_t item_num)
{
        ...
        */
        ...
        {
                item->hit_points += (g_Input & IN_SLOW ? -2 : 2) * LARA_HITPOINTS / 100; // change by 2%
...
if (g_InputDB.item_cheat)
		...
		*/
        ...
        case LWS_CHEAT:
                ...
                g_Lara.death_count = 0;
                LaraUnderWater(item, &coll);
                //if (g_Input & IN_SLOW && !(g_Input & IN_LOOK) && !g_Input.fly_cheat)
...
void AnimateLara(ITEM_INFO *item)
{
        ...
        if (item->frame_number > anim->frame_end)
        {
                if (anim->number_commands > 0)
                {
                        ...
                        for (int32_t i = 0; i < anim->number_commands; i++)
                        {
                                switch (*command++)
                                {
                                case AC_MOVE_ORIGIN:
...
void UseItem(int16_t object_num)
{
        ...
        case O_GUN_OPTION:
                ...
                if (g_Lara.gun_status == LGS_ARMLESS && g_Lara.gun_type == LGT_PISTOLS)
                {
        ...
        case O_SHOTGUN_OPTION:
                ...
                if (g_Lara.gun_status == LGS_ARMLESS && g_Lara.gun_type == LGT_SHOTGUN)
                {
                        g_Lara.gun_type = LGT_UNARMED;
                }
                break;
        ...
        case O_MAGNUM_OPTION:
                ...
                if (g_Lara.gun_status == LGS_ARMLESS && g_Lara.gun_type == LGT_MAGNUMS)
                {
                        g_Lara.gun_type = LGT_UNARMED;
                }
                break;
...
void LaraInitialiseMeshes(int32_t level_num)
{
        ...
        if (start->costume)
        {
                for (int32_t i = 0; i < LM_NUMBER_OF; i++)
                {
                        ...
                        g_Lara.mesh_ptrs[i] =
                                g_Meshes[g_Objects[use_orig_mesh ? O_LARA : O_LARA_EXTRA]
                                                         .mesh_index +
                                                 i];
                }
                return;
