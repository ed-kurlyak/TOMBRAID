...
int16_t GunShot(int32_t x, int32_t y, int32_t z, int16_t speed, PHD_ANGLE y_rot,
				int16_t room_num)
{
...
int16_t GunHit(int32_t x, int32_t y, int32_t z, int16_t speed, PHD_ANGLE y_rot,
			   int16_t room_num)
{
	PHD_VECTOR pos;
	pos.x = 0;
	...
	DoBloodSplat(pos.x, pos.y, pos.z, g_LaraItem->speed, g_LaraItem->pos.y_rot,
				 g_LaraItem->room_number);
	Sound_Effect(SFX_LARA_BULLETHIT, &g_LaraItem->pos, SPM_NORMAL);
	return GunShot(x, y, z, speed, y_rot, room_num);
}

int16_t GunMiss(int32_t x, int32_t y, int32_t z, int16_t speed, PHD_ANGLE y_rot,
				int16_t room_num)
{
	GAME_VECTOR pos;
