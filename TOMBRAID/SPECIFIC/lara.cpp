...
void LaraAsWalk(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	if (g_Input & IN_FORWARD)
	{
		item->goal_anim_state = (g_Input & IN_SLOW) ? AS_WALK : AS_RUN;
	}
	else
	{
		item->goal_anim_state = AS_STOP;
...
void LaraAsStop(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	{
		g_Camera.type = CAM_LOOK;
		if (g_Input & IN_LEFT && g_Lara.head_y_rot > -MAX_HEAD_ROTATION)
		{
			g_Lara.head_y_rot -= HEAD_TURN / 2;
		}
		else if (g_Input & IN_RIGHT && g_Lara.head_y_rot < MAX_HEAD_ROTATION)
		{
			g_Lara.head_y_rot += HEAD_TURN / 2;
		}
		g_Lara.torso_y_rot = g_Lara.head_y_rot;

		if (g_Input & IN_FORWARD && g_Lara.head_x_rot > MIN_HEAD_TILT_LOOK)
		{
			g_Lara.head_x_rot -= HEAD_TURN / 2;
		}
		else if (g_Input & IN_BACK && g_Lara.head_x_rot < MAX_HEAD_TILT_LOOK)
		{
			g_Lara.head_x_rot += HEAD_TURN / 2;
		}
		g_Lara.torso_x_rot = g_Lara.head_x_rot;
...
void LaraAsTurnR(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	/*
if (g_Config.enable_enhanced_look && g_Input & IN_LOOK) {
	item->goal_anim_state = AS_STOP;
	return;
}
	*/
...
void LaraAsCompress(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	else if (g_Input & IN_LEFT && LaraFloorFront(item, item->pos.y_rot - PHD_90,
											256) >= -STEPUP_HEIGHT)
	{
		item->goal_anim_state = AS_LEFTJUMP;
		g_Lara.move_angle = item->pos.y_rot - PHD_90;
	}
	else if (g_Input & IN_RIGHT && LaraFloorFront(item, item->pos.y_rot + PHD_90,
											 256) >= -STEPUP_HEIGHT)
	{
		item->goal_anim_state = AS_RIGHTJUMP;
		g_Lara.move_angle = item->pos.y_rot + PHD_90;
	}
	else if (g_Input & IN_BACK && LaraFloorFront(item, item->pos.y_rot - PHD_180,
											256) >= -STEPUP_HEIGHT)
	{
...
void LaraAsBack(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	item->goal_anim_state = g_Input & IN_BACK && g_Input & IN_SLOW ? AS_BACK : AS_STOP;
...
void LaraColWalk(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	if (LaraDeflectEdge(item, coll))
	{
		...
		else if ((item->frame_number >= 22 && item->frame_number <= 28) ||
...
void LaraColHang(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	if (item->goal_anim_state == AS_HANG && g_Input & IN_FORWARD)
	{
		if (coll->front_floor > -850 && coll->front_floor < -650 &&
			coll->front_floor - coll->front_ceiling >= 0 &&
			coll->left_floor - coll->left_ceiling >= 0 &&
			coll->right_floor - coll->right_ceiling >= 0 && !coll->hit_static)
		{
			item->goal_anim_state = (g_Input & IN_SLOW) ? AS_GYMNAST : AS_NULL;
		}
	}
}
...
void LaraColLand(ITEM_INFO *item, COLL_INFO *coll) { LaraColStop(item, coll); }
...
void LaraColUpJump(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	GetCollisionInfo(coll, item->pos.x, item->pos.y, item->pos.z,
					 item->room_number, 870);
...
bool TestLaraVault(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	else if (angle >= (PHD_180 - 1) - VAULT_ANGLE ||
	...
	else if (hdif >= -STEP_L * 3 - STEP_L / 2 &&
	...
	else if (hdif >= -STEP_L * 7 - STEP_L / 2 &&
...
bool LaraTestHangJump(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	else if (angle >= (PHD_180 - 1) - HANG_ANGLE ||
...
bool LaraLandedBad(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	int32_t landspeed = item->fall_speed - DAMAGE_START;
	if (landspeed <= 0)
	{
		return false;
	}
