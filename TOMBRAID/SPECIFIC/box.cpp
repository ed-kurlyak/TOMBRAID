...
/*
собирает информацию о положении Лары относительно существа
...
void CreatureAIInfo(ITEM_INFO *item, AI_INFO *info)
{
        ...
        //pivot_length расстояние от центра льва (модели)
        //до пасти (головы) по оси Z (то есть вперед)
        ...
        //угол между Ларой и врагом в мировой системе угол
        PHD_ANGLE angle = phd_atan(z, x);

        //расстояние от Лары до врага
        ...
        //угол от врага к Ларе
        info->angle = angle - item->pos.y_rot;

        //угол между Ларой и врагом относительно направления Лары
        //плюс 180° что бы угол развернуть в обратную сторону от Лары к врагу
        //так как угол angle направлен от врага к Ларе
...
int32_t SearchLOT(LOT_INFO *LOT, int32_t expansion)
{
        ...
        for (int32_t i = 0; i < expansion; i++)
        {
                ...
                int32_t done = 0;
                int32_t index = box->overlap_index & OVERLAP_INDEX;
                ...
                do
                {
                        ...
                        int32_t change = g_Boxes[box_number].height - box->height;
...
int32_t StalkBox(ITEM_INFO *item, int16_t box_number)
{
        ...
        int32_t enemy_quad = (g_LaraItem->pos.y_rot >> 14) + 2;
        int32_t box_quad = (z > 0) ? ((x > 0) ? 2 : 1) : ((x > 0) ? 3 : 0);
        ...
        int32_t baddie_quad = (item->pos.z > g_LaraItem->pos.z)
