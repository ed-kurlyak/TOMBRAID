...
void Option_Sound(INVENTORY_ITEM *inv_item)
{
	...
	if (g_InputDB & IN_FORWARD && g_OptionSelected > TEXT_OPTION_MIN)
	{
	...
	if (g_InputDB & IN_BACK && g_OptionSelected < TEXT_OPTION_MAX)
	{
		Text_RemoveOutline(m_Text[g_OptionSelected]);
		Text_RemoveBackground(m_Text[g_OptionSelected]);
		Text_AddBackground(m_Text[++g_OptionSelected], 128, 0, 0, 0);
	...
	case TEXT_MUSIC_VOLUME:
		if (g_Input & IN_LEFT && g_Config.music_volume > 0)
		{
		...
		else if (g_Input & IN_RIGHT && g_Config.music_volume < 10)
		{
			g_Config.music_volume++;
			g_IDelay = 1;
			g_IDCount = 10;
		...
		if (g_Input & IN_LEFT || g_Input & IN_RIGHT)
		{
	...
	case TEXT_SOUND_VOLUME:
		if (g_Input & IN_LEFT && g_Config.sound_volume > 0)
		{
		...
		else if (g_Input & IN_RIGHT && g_Config.sound_volume < 10)
		{
			g_Config.sound_volume++;
			g_IDelay = 1;
			g_IDCount = 10;
