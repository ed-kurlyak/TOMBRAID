...
#include "..\\traps\\dart.h"
#include "..\\traps\\dart_emi.h"
#include "..\\traps\\fal_blok.h"
#include "..\\traps\\mov_blok.h"
#include "..\\traps\\pendul.h"
#include "..\\traps\\da_sword.h"
#include "..\\traps\\fal_ceil.h"
#include "..\\traps\\rol_ball.h"
#include "..\\traps\\spikes.h"
#include "..\\traps\\tee_trap.h"
#include "..\\traps\\ligh_emi.h"
#include "..\\traps\\mid_touc.h"
#include "..\\traps\\rol_blok.h"
#include "..\\traps\\tor_hamm.h"
#include "..\\traps\\flame.h"
#include "..\\traps\\lava.h"
#include "..\\effects\\bod_part.h"
#include "..\\effects\\missile.h"
#include "..\\effects\\ricochet.h"
#include "..\\effects\\splash.h"
#include "..\\effects\\twinkle.h"
#include "..\\effects\\watrfall.h"
#include "box.h"
#include "items.h"
#include "sound.h"
...
bool S_LoadLevel(char *szLevelName)
{
        ...
        fp = fopen(szLevelName, "rb");

        if (!fp)
        {
                printf("S_LoadLevel(): Could not open level file");
                return false;
        }
        ...
        if (LevelVersion != 32)
        {
                printf("Wrong Tomb Raider level version");
...
bool Load_Texture_Pages(FILE *fp)
{
        ...
        for (unsigned int i = 0; i < TexturePageCount; i++)
        {
...
bool Load_Rooms(FILE *fp)
{
        ...
        if (g_RoomCount > 1024)
        {
                printf("Load_Rooms(): Too many rooms"); 
                return false;
        }
...
bool LoadBoxes(FILE *fp)
{
        ...
        {
                printf("LoadBoxes(): Unable to load boxes");
        ...
        {
                printf("LoadBoxes(): Unable to load box overlaps");
                return false;
        }
        ...
        for (int i = 0; i < 2; i++)
        {
                ...
                {
                        printf("LoadBoxes(): Unable to load 'ground_zone'");
                        return false;
                }
                ...
                {
                        printf("LoadBoxes(): Unable to load 'ground2_zone'");
                        return false;
                }
                ...
                {
                        printf("LoadBoxes(): Unable to load 'fly_zone'");
                        return false;
                }
        }
...
bool LoadItems(FILE *fp)
{
        ...
        if (ItemCount)
        {
                ...
                {
                        printf("LoadItems(): Too Many g_Items being Loaded!");
                        return false;
                }
                ...
                for (int i = 0; i < ItemCount; i++)
                {
                        ...
                        {
                                printf("LoadItems(): Bad Object number on Item");
...
bool LoadSoundSamples(FILE *fp)
{
        ...
        for (i = 0; i < NumSamples; i++)
        {
                //memcpy(dwHeader, SamplePointers[i], sizeof(unsigned int) * 11);
/*
...
/*
                if (FAILED(DS_MakeSample(
                                i, pWF,
                                //пропускаем заголовок WAV файла sizeof(unsigned int) * 11
                                (unsigned char *)SamplePointers[i] + sizeof(unsigned int) * 11,
                                dwWaveLength)))
                {
                        printf("DS_MakeSample Error!"); 
                }
                */
			
			/*
			unsigned int sample_size = SampleSizes[i];
			Sound_Buff[i].Buff_data = (unsigned char*)malloc(sample_size);
			memcpy((void*)Sound_Buff[i].Buff_data, (void*)SamplePointers[i], sample_size);
