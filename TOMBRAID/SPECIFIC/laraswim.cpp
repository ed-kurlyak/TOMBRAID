...
void LaraAsUWDeath(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	if (item->pos.x_rot >= -2 * PHD_DEGREE && item->pos.x_rot <= 2 * PHD_DEGREE)
	{
		item->pos.x_rot = 0;
	}
...
void LaraSwimCollision(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	GetCollisionInfo(coll, item->pos.x, item->pos.y + UW_HITE / 2, item->pos.z,
...
void LaraWaterCurrent(COLL_INFO *coll)
{
	...
	FLOOR_INFO *floor =
		&r->floor[((item->pos.z - r->z) >> WALL_SHIFT) +
