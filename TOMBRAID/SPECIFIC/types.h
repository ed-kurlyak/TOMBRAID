...
//исправил типы signed 
//падала коллизия после запуска уровня
/*
typedef struct COLL_INFO
{
	signed int mid_floor;
	signed int mid_ceiling;
	signed int mid_type;
	signed int front_floor;
	signed int front_ceiling;
	signed int front_type;
	signed int left_floor;
	signed int left_ceiling;
	signed int left_type;
	signed int right_floor;
	signed int right_ceiling;
	signed int right_type;
	signed int radius;
	signed int bad_pos;
	signed int bad_neg;
	signed int bad_ceiling;
	PHD_VECTOR shift;
	PHD_VECTOR old;
	signed short int facing;
	signed short int quadrant;
	signed short int coll_type;
	signed short int* trigger;
	signed char tilt_x;
	signed char tilt_z;
	signed char hit_by_baddie;
	signed char hit_static;
	unsigned short int slopes_are_walls : 1;
	unsigned short int slopes_are_pits : 1;
	unsigned short int lava_is_pit : 1;
	unsigned short int enable_baddie_push : 1;
	unsigned short int enable_spaz : 1;
} COLL_INFO;
*/
...
typedef struct REQUEST_INFO
{
        ...
        uint16_t line_old_offset; // Предыдущее значение offset — может быть для
                                                          // сравнения
        ...
        //uint16_t item_flags[MAX_REQLINES]; // флаги для строк (например, блокировка
                                                                           // выбора)
} REQUEST_INFO;
