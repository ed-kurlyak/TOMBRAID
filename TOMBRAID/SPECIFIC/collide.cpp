...
void GetCollisionInfo(COLL_INFO *coll, int32_t xpos, int32_t ypos, int32_t zpos,
					  int16_t room_num, int32_t objheight)
{
	...
	else if (coll->slopes_are_pits && coll->front_type == HT_BIG_SLOPE &&
			 coll->front_floor > 0)
	{
		coll->front_floor = 512;
	}
	else if (coll->lava_is_pit && coll->front_floor > 0 && g_TriggerIndex &&
	...
	else if (coll->slopes_are_pits && coll->left_type == HT_BIG_SLOPE &&
			 coll->left_floor > 0)
	{
		coll->left_floor = 512;
	}
	else if (coll->lava_is_pit && coll->left_floor > 0 && g_TriggerIndex &&
			 (g_TriggerIndex[0] & DATA_TYPE) == FT_LAVA)
	{
		coll->left_floor = 512;
	}
	...
	else if (coll->slopes_are_pits && coll->right_type == HT_BIG_SLOPE &&
			 coll->right_floor > 0)
	{
		coll->right_floor = 512;
	}
	else if (coll->lava_is_pit && coll->right_floor > 0 && g_TriggerIndex &&
			 (g_TriggerIndex[0] & DATA_TYPE) == FT_LAVA)
	{
		coll->right_floor = 512;
	}
	...
	if (coll->front_floor > coll->bad_pos ||
		coll->front_floor < coll->bad_neg ||
	...
	if (coll->right_floor > coll->bad_pos || coll->right_floor < coll->bad_neg)
	{
		switch (coll->quadrant)
		{
		case DIR_NORTH:
...
int32_t CollideStaticObjects(COLL_INFO *coll, int32_t x, int32_t y, int32_t z,
							 int16_t room_number, int32_t hite)
{
	PHD_VECTOR shifter;
	...
	for (int32_t i = 0; i < g_RoomsToDrawCount; i++)
	{
		...
		for (int32_t j = 0; j < r->num_static_meshes; j++, mesh++)
		{
...
void GetNearByRooms(int32_t x, int32_t y, int32_t z, int32_t r, int32_t h,
					int16_t room_num)
{
...
int16_t GetTiltType(FLOOR_INFO *floor, int32_t x, int32_t y, int32_t z)
{
	...
	while (floor->pit_room != NO_ROOM)
	{
		...
		floor = &r->floor[((z - r->z) >> WALL_SHIFT) +
...
void LaraBaddieCollision(ITEM_INFO *lara_item, COLL_INFO *coll)
{
	...
	if (door)
	{
		for (int32_t i = 0; i < door->count; i++)
		{
...
void ItemPushLara(ITEM_INFO *item, ITEM_INFO *lara_item, COLL_INFO *coll,
				  int32_t spazon, int32_t bigpush)
{
	int32_t x = lara_item->pos.x - item->pos.x;
	int32_t z = lara_item->pos.z - item->pos.z;
	...
	if (rx >= minx && rx <= maxx && rz >= minz && rz <= maxz)
	{
		...
		coll->facing = phd_atan(lara_item->pos.z - coll->old.z,
								lara_item->pos.x - coll->old.x);
		GetCollisionInfo(coll, lara_item->pos.x, lara_item->pos.y,
						 lara_item->pos.z, lara_item->room_number, LARA_HITE);
