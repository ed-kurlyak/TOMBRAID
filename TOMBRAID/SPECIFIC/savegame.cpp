...
#include "game/objects/puzz_hol.h"
#include "game/shell.h"
#include "game/traps/mov_blok.h"
#include "game/traps/rol_blok.h"
...
#include "lot.h"
//#include "game/shell.h"
#include "../traps/mov_blok.h"
#include "../traps/rol_blok.h"
#include "const.h"
#include "vars.h"
...
void ExtractSaveGameInfo()
{
        ...
        for (i = 0; i < g_SaveGame.num_pickup1; i++)
        {
                Inv_AddItem(O_PICKUP_ITEM1);
        }

        for (i = 0; i < g_SaveGame.num_pickup2; i++)
        {
                Inv_AddItem(O_PICKUP_ITEM2);
        }

        for (i = 0; i < g_SaveGame.num_puzzle1; i++)
        {
                Inv_AddItem(O_PUZZLE_ITEM1);
        }

        for (i = 0; i < g_SaveGame.num_puzzle2; i++)
        {
                Inv_AddItem(O_PUZZLE_ITEM2);
        }

        for (i = 0; i < g_SaveGame.num_puzzle3; i++)
        {
                Inv_AddItem(O_PUZZLE_ITEM3);
        }

        for (i = 0; i < g_SaveGame.num_puzzle4; i++)
        {
                Inv_AddItem(O_PUZZLE_ITEM4);
        }

        for (i = 0; i < g_SaveGame.num_key1; i++)
        {
                Inv_AddItem(O_KEY_ITEM1);
        }

        for (i = 0; i < g_SaveGame.num_key2; i++)
        {
                Inv_AddItem(O_KEY_ITEM2);
        }

        for (i = 0; i < g_SaveGame.num_key3; i++)
        {
                Inv_AddItem(O_KEY_ITEM3);
        }

        for (i = 0; i < g_SaveGame.num_key4; i++)
        {
                Inv_AddItem(O_KEY_ITEM4);
        }

        for (i = 0; i < g_SaveGame.num_leadbar; i++)
        {
