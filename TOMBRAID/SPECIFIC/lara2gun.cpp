...
void AnimatePistols(int32_t weapon_type)
{
	...
	if (g_Lara.right_arm.lock || (g_Input & IN_ACTION && !g_Lara.target))
	{
		...
		else if (anir == AF_G_AIM_L && g_Input & IN_ACTION)
		{
			//стрельба, отдача
	...
	if (g_Lara.left_arm.lock || (g_Input & IN_ACTION && !g_Lara.target))
	{
		if (anil >= AF_G_AIM && anil < AF_G_AIM_L)
		{
			anil++;
		}
		else if (anil == AF_G_AIM_L && g_Input & IN_ACTION)
		{
