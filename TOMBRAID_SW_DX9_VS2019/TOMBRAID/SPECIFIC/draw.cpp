					   ...
					   int16_t shade)
{
	...
	//для случая если z-buff не линейный
...
do {                                                                \
    ...
    OUT_BUCKET.Vertex[OUT_BUCKET.count].tv = IN_VERTICES.v;      \
    OUT_BUCKET.count++;                                             \
} while(0)


#define COLOR_VERT_BUFF_TO_DX_BUFFER(OUT_BUCKET, IN_VERTICES, IN_COLOR)  \
do {                                                                \
    OUT_BUCKET.Vertex[OUT_BUCKET.count].x = IN_VERTICES.x;       \
    OUT_BUCKET.Vertex[OUT_BUCKET.count].y = IN_VERTICES.y;       \
    OUT_BUCKET.Vertex[OUT_BUCKET.count].z = IN_VERTICES.z;       \
		...
		if (vns[0]->clip >= 0 && vns[1]->clip >= 0 && vns[2]->clip >= 0)
		{
				...
				0)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					float f_oneopersp = one / g_PhdPersp;   // нормализация в fixed-point диапазон
					ooz *= f_oneopersp;
					*/
...
int16_t* S_DrawObjectGT4_HW(int16_t* obj_ptr, int32_t number)
{
	...
	if (number > 0)
	{
		do
		{
			...
			//для ClipVertices в экранных координатах важно что бы
			//на входе был треугольник, иначе искажения цвета/текстур
			//или переделать ClipVertices для полигона TriangleStrip
