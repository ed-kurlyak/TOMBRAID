...
void LaraAsCompress(ITEM_INFO *item, COLL_INFO *coll)
{
	...
	else if (g_Input.left && LaraFloorFront(item, item->pos.y_rot - PHD_90,
											256) >= -STEPUP_HEIGHT)
	{
		item->goal_anim_state = AS_LEFTJUMP;
		g_Lara.move_angle = item->pos.y_rot - PHD_90;
	}
	else if (g_Input.right && LaraFloorFront(item, item->pos.y_rot + PHD_90,
											 256) >= -STEPUP_HEIGHT)
	{
		item->goal_anim_state = AS_RIGHTJUMP;
		g_Lara.move_angle = item->pos.y_rot + PHD_90;
	}
	else if (g_Input.back && LaraFloorFront(item, item->pos.y_rot - PHD_180,
											256) >= -STEPUP_HEIGHT)
	{
