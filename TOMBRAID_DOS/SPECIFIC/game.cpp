...
void Input_Update()
{
	...
	if (Get_Key_State(4))
		//g_Input.any |= IN_STEPL;
		g_Input.step_left = 1;

	if (Get_Key_State(5))
		//g_Input.any |= IN_STEPR;
	...
	if (Get_Key_State(12) && g_Camera.type != CAM_CINEMATIC)
		//g_Input.any |= IN_OPTION;
		g_Input.option = 1;

	if (g_Input.forward & IN_FORWARD && g_Input.back & IN_BACK)
		//g_Input.any |= IN_ROLL;
		g_Input.roll = 1;


	if (Keys_States_Array[KEY_RETURN] != 0 || (g_Input.any & IN_ACTION))
	{
		//g_Input.any |= IN_SELECT;
		g_Input.select = 1;
	}

	if (Keys_States_Array[KEY_ESCAPE] != 0)
		//g_Input.any |= IN_DESELECT;
		g_Input.deselect = 1;

	/* Make left+right cancel */
	if ((g_Input.any & (IN_LEFT | IN_RIGHT)) == (IN_LEFT | IN_RIGHT))
		g_Input.any -= (IN_LEFT | IN_RIGHT);
